...
function focusin(event:Event) {
    ...
    if (target.tagName === 'TEXTAREA' || (target.tagName === 'INPUT' && target.type === 'text') || (target.tagName === 'DIV' && target.getAttribute('data-text-input') === 'true') || target.getAttribute("contenteditable") === 'true') {
        // 检查是否已经有翻译容器，如果没有则添加
        let nextSibling = target.nextElementSibling;
        if (!nextSibling || !nextSibling.classList.contains('translate-container')) {
